  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4">
        {Object.entries(requestsByType).map(
          ([type, requests]: [string, any]) => (
            <Card key={type} className="bg-slate-800/50 border-slate-700">
              <CardContent className="p-3 sm:p-4">
                <div className="flex items-center space-x-2">
                  {getTypeIcon(type)}
                  <div>
                    <p className="text-xs sm:text-sm font-medium text-slate-400 capitalize">
                      {type}
                    </p>
                    ...
                    <p className="text-xs text-slate-500">
                      {(
                        requests.reduce(
                          (acc: number, req: any) => acc + req.size,
                          0,
                        ) /
                        1024 /
                        1024
          ...
          <Card className="bg-slate-800/50 border-slate-700">
            <CardHeader>
              ...
              <CardDescription className="text-slate-400">
                WASM compilation and execution performance
              </CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <p className="text-slate-100 font-medium">
                          {module.compileTime}ms
                        </p>
                        <Progress
                          value={(module.compileTime / 200) * 100}
                          className="mt-1"
                        />
                      </div>
                      <div>
                        ...
                        <p className="text-slate-100 font-medium">
                          {module.instantiateTime}ms
                        </p>
                        <Progress
                          value={(module.instantiateTime / 100) * 100}
                          className="mt-1"
                        />
                      </div>
                    </div>
                    ...
                    <div className="bg-slate-700/50 rounded p-3">
                      ...
                      <p className="text-slate-100">
                        {(module.memoryUsage / 1024 / 1024).toFixed(1)}MB
                      </p>
                    </div>
                  </div>
                ))}
        ...
        <TabsContent value="glb">
          <Card className="bg-slate-800/50 border-slate-700">
            <CardHeader>
              ...
              <CardDescription className="text-slate-400">
                3D model complexity and loading performance
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                ...
                <div className="space-y-3">
                  {Object.entries(requestsByType).map(
                    ([type, requests]: [string, any]) => {
                      ...
                      const avgDuration =
                        requests.reduce(
                          (acc: number, req: any) => acc + req.duration,
                          0,
                        ) / requests.length;
            ...
            <Card className="bg-slate-800/50 border-slate-700">
              <CardHeader>
                ...
                <CardDescription className="text-slate-400">
                  Resource impact on page performance
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  <div className="bg-red-900/20 border border-red-800 rounded-lg p-3">
                    ...
                    <div className="space-y-1 text-sm">
                      <p className="text-red-200">
                        WASM modules: {data.wasmModules.length} files
                      </p>
                      <p className="text-red-200">
                        GLB files: {data.glbFiles.length} files
                      </p>
                      <p className="text-red-200">
                        Large scripts:{" "}
                        {requestsByType.script?.filter(
                          (r: any) => r.size > 100000,
                        ).length || 0}{" "}
                        files
                      </p>
                  ...
                  <div className="bg-yellow-900/20 border border-yellow-800 rounded-lg p-3">
                    ...
                    <div className="space-y-1 text-sm">
                      <p className="text-yellow-200">
                        Images: {requestsByType.image?.length || 0} files
                      </p>
                      <p className="text-yellow-200">
                        Stylesheets: {requestsByType.stylesheet?.length || 0}{" "}
                        files
                      </p>
                      <p className="text-yellow-200">
                        XHR/Fetch:{" "}
                        {(requestsByType.xhr?.length || 0) +
                          (requestsByType.fetch?.length || 0)}{" "}
                        requests
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="bg-green-900/20 border border-green-800 rounded-lg p-3">
                    ...
                    <div className="space-y-1 text-sm">
                      <p className="text-green-200">
                        Fonts: {requestsByType.font?.length || 0} files
                      </p>
                      <p className="text-green-200">
                        Other: {requestsByType.other?.length || 0} files
                      </p>
                      <p className="text-green-200">
                        Documents: {requestsByType.document?.length || 0} files
                      </p>
                    </div>
                  </div>
                </div>
