...
export function PerformanceTimelineChart({ data }: PerformanceTimelineChartProps) {
  const [timelineZoom, setTimelineZoom] = useState({ start: 0, end: 100 }) // Show full timeline initially
  const [isPlaying, setIsPlaying] = useState(false)
  const [currentTime, setCurrentTime] = useState(0)
  const [isFullscreen, setIsFullscreen] = useState(false)

  // Calculate time range and statistics from REAL data
        ...
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              <Badge variant="outline" className="text-xs bg-green-700 border-green-600 text-green-300">
                Complete Timeline
              </Badge>
            </div>
          </div>
